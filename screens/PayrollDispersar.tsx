...
import {  GET_PAYROLLCOLLABORATOR_DISPERSION, GET_PAYROLL,GET_ALL_DISPERSIONLAYOUT } from "../Querys/querys";
...
const PayrollTimbrar = (props: TabPanelProps) => {
  ...
  const [totalADisersar, setTotalaDispersar] = useState<any[]>([]) 
  const [barraProgreso, setBarraProgreso] = useState<any[]>([]) 
  const history = useHistory();

  const {data: dataDipsersion, loading: loadingDispersion, startPolling, stopPolling} = useQuery(GET_PAYROLLCOLLABORATOR_DISPERSION, {
    variables: { getPayrollcollaboratorDispersionId: id },
  });
  ...
  const allPayrollDispersion = dataDipsersion?.GET_PAYROLLCOLLABORATOR_DISPERSION;
  ...
  const actualizaSaldos = ( ) =>{
    ...
    }).map((lis:any) =>(lis.TotalIncomeNotTaxable)))
        ...
        <table className={styles.pd_tablaPrincipal}>
          ...
          {filtrado?.map((lis:any, index:Number)=>(
              <tr>
                ...
                <td className={styles.pd_columnaDatos}>{formatter(lis.netIncomeTaxable)}</td>
                <td className={styles.pd_columnaDatos}>{formatter(lis.TotalIncomeNotTaxable)}</td>
