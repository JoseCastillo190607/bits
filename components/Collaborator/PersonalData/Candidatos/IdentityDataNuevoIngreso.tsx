...
const validateRfcAPI = require('validate-rfc');
...
const IdentityDataNuevoIngreso = () => {
    ...
    const handleChangeRfc = async (e: ChangeEvent<{ name: string, value: unknown }>) => {
        ...
        if(state.collaborator?.RFC != undefined){
            ...
            const validado =validateRfcAPI(state.collaborator?.RFC)
            //console.log(state.collaborator?.RFC.toLocaleUpperCase());
            
            //console.log(validado.isValid);
            let Mensaje = ''
            if(!validado.isValid){
                if(validado.errors == 'INVALID_FORMAT'){
                    Mensaje = 'El formato de tu RFC no es la correcta.'
                }
                if(validado.errors == 'INVALID_DATE'){
                    Mensaje = 'El formato de la fecha de tu RFC no es la correcta.'
                }
                if(validado.errors == 'INVALID_VERIFICATION_DIGIT'){
                    Mensaje = 'El último caracter (dígito verificador) es inválido.'
                }
                if(validado.errors == 'FORBIDDEN_WORD'){
                    Mensaje = 'El RFC contiene una de las palabras inconvenientes que no pueden formar un RFC.'
                }
                if(Mensaje == ''){
                    Mensaje = 'Verifique RFC'
                }
            }
            
            if (!validado.isValid) {setValidatdRfc(false); validateRFC = false; return await WarningAlert({ text: Mensaje });}
    ...
    console.log("a", adminState?.PermisosContex?.Modulos?.Colaboradores?.NuevoIngreso?.Ver?.DatosPersonales?.DatosIdentidad?.Editar)
    // adminState?.PermisosContex?.Modulos?.Colaboradores?.NuevoIngreso?.Ver?.DatosIdentidad.Editar
...
export default IdentityDataNuevoIngreso;